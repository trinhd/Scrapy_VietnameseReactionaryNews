...
class QuanLamBaoSpider(scrapy.Spider):
	name = "quanlambao"
	allowed_domains = ["quanlambao.blogspot.com"]
	start_urls = (
	...
	denyLinks = [
		...
		re.compile(r'.*quanlambao\.blogspot\.com\/search.*'),
		re.compile(r'https:\/\/quanlambao\.blogspot\.com.*'),
	]
	allowLinks = [
		#re.compile(r'.*\/tin\-tuc\/giao\-duc.*'),
		re.compile(r'.*quanlambao\.blogspot\.com.*'),
	]
	...
	def parse(self, response):
		...
		if author == None:
			author = response.xpath('//span[@style="color: #20124d;"]').extract_first()
			if author == None:
				author = response.xpath('//span[@style="color: blue;"]').extract_first()
				if author == None:
					author = response.xpath('//span[@style="color: #256ead;"]').extract_first()
					if author == None:
						return
